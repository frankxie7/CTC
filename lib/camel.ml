...
let draw_camel r t =
  (* TODO : theres something to do here - we don't need to draw the camel if it
     is the idle animation, and row + frame_height calculations are weird*)
  let src =
    Rect.create
      ~x:(camel_init_height + frame_height)
      ~y:(camel_init_width + frame_width)
